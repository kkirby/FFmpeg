...
static char *choose_pix_fmts(OutputStream *ost)
{
    ...
    if (ost->enc_ctx->pix_fmt != AV_PIX_FMT_NONE) {
        ...
        if (avio_open_dyn_buf(&s) < 0)
            exit_program(2, "Unable to open avio buffer");
...
{                                                                              \
    if (ost->enc_ctx->var != none) {                                           \
        ...
        int len;                                                               \
                                                                               \
        if (avio_open_dyn_buf(&s) < 0)                                         \
            exit_program(2, "Unable to open avio dyn buff");                   \
                                                                               \
...
FilterGraph *init_simple_filtergraph(InputStream *ist, OutputStream *ost)
{
    ...
    if (!fg)
        exit_program(2, "Unable to allocate avz");
...
static void init_input_filter(FilterGraph *fg, AVFilterInOut *in)
{
    ...
    if (type != AVMEDIA_TYPE_VIDEO && type != AVMEDIA_TYPE_AUDIO) {
        av_log(NULL, AV_LOG_FATAL, "Only video and audio filters supported "
               "currently.\n");
        exit_program(2, "Only video and audio filters supported "
               "currently.");
    }
    ...
    if (in->name) {
        ...
        if (file_idx < 0 || file_idx >= nb_input_files) {
            ...
            exit_program(2, "Invalid file index %d in filtergraph description %s.",
                   file_idx, fg->graph_desc);
        }
        ...
        if (!st) {
            av_log(NULL, AV_LOG_FATAL, "Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.\n", p, fg->graph_desc);
            exit_program(2, "Stream specifier '%s' in filtergraph description %s "
                   "matches no streams.", p, fg->graph_desc);
        }
        ...
        if (i == nb_input_streams) {
            av_log(NULL, AV_LOG_FATAL, "Cannot find a matching stream for "
                   ...
                   in->filter_ctx->name);
            exit_program(2, "Cannot find a matching stream for "
    ...
    if (!(fg->inputs[fg->nb_inputs - 1] = av_mallocz(sizeof(*fg->inputs[0]))))
        exit_program(2, "Unable to alloc avz");
...
int init_complex_filtergraph(FilterGraph *fg)
{
    ...
    for (cur = outputs; cur;) {
        ...
        if (!fg->outputs[fg->nb_outputs - 1])
            exit_program(2, "No output matching nboutputs - 1");
...
int configure_output_filter(FilterGraph *fg, OutputFilter *ofilter, AVFilterInOut *out)
{
    ...
    if (!ofilter->ost) {
        ...
        exit_program(2, "Filter %s has a unconnected output", ofilter->name);
    }
